...
public class Yolov5TFLiteDetector {
    ...
    private final Size INPNUT_SIZE = new Size(640, 640);//输入的大小
    private final int[] OUTPUT_SIZE0 = new int[]{1, 80,80,13,3};//输出1
    private final int[] OUTPUT_SIZE1 = new int[]{1,40,40,13,3};//输出2
    private final int[] OUTPUT_SIZE2 = new int[]{1,20,20,13,3};//输出3
    ...
    private final String MODEL_YOLOV5S = "best.tflite";//使用的模型
//    private final String MODEL_YOLOV5S = "yolov5s-dynamic.tflite";
    ...
    private final String LABEL_FILE = "best_label.txt";
    ...
    public int[] getOutputSize(){return this.OUTPUT_SIZE0;}
    ...
    public ArrayList<Recognition> detect(Bitmap bitmap) {
        ...
        Object[] inputArray={yolov5sTfliteInput.getBuffer()};


/*        yolov5sTfliteInput.load(bitmap);
        yolov5sTfliteInput = imageProcessor.process(yolov5sTfliteInput[0]);
*/

        // yolov5s-tflite的输出是:[1, 6300, 85], 可以从v5的GitHub release处找到相关tflite模型, 输出是[0,1], 处理到320.
        Map<Integer,Object> probabilityBuffer=new HashMap<>();
            TensorBuffer classesOutput0=TensorBuffer.createFixedSize(OUTPUT_SIZE0,DataType.FLOAT32);
            TensorBuffer classesOutput1=TensorBuffer.createFixedSize(OUTPUT_SIZE1,DataType.FLOAT32);
            TensorBuffer classesOutput2=TensorBuffer.createFixedSize(OUTPUT_SIZE2,DataType.FLOAT32);
        probabilityBuffer.put(0,classesOutput0.getBuffer());
        probabilityBuffer.put(1,classesOutput1.getBuffer());
        probabilityBuffer.put(2,classesOutput2.getBuffer());

            tflite.runForMultipleInputsOutputs(inputArray, probabilityBuffer);
...
*/
        float[] classOutputArray0=classesOutput0.getFloatArray();
        float[] classOutputArray1=classesOutput1.getFloatArray();
        float[] classOutputArray2=classesOutput2.getFloatArray();

        // 输出数据被平铺了出来

        float[] recognitionArray = classesOutput0.getFloatArray();
        // 这里将flatten的数组重新解析(xywh,obj,classes).
        ArrayList<Recognition> allRecognitions = new ArrayList<>();
        for (int i = 0; i < OUTPUT_SIZE2[1]; i++) {
            int gridStride = i * OUTPUT_SIZE2[2];
            ...
            float[] classScores = Arrays.copyOfRange(recognitionArray, 5 + gridStride, this.OUTPUT_SIZE0[2] + gridStride);
//            if(i % 1000 == 0){
...
//        Log.i("tfliteSupport", "recognize data size: "+allRecognitions.size());
    ...
    protected ArrayList<Recognition> nms(ArrayList<Recognition> allRecognitions) {
        ...
        // 遍历每个类别, 在每个类别下做nms
        for (int i = 0; i < OUTPUT_SIZE2[2]-5; i++) {
